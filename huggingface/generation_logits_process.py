...
class EtaLogitsWarper(LogitsWarper):
    r"""
    :class:`transformers.LogitsWarper` that performs eta-sampling (https://aclanthology.org/2022.findings-emnlp.249/).
    ...
    def __init__(self, epsilon: float, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1):
        if not isinstance(epsilon, float) or epsilon <= 0:
            raise ValueError(f"`top_k` has to be a strictly positive float, but is {epsilon}")
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        # the scores here is the raw logits, have to go through softmax for computing eta
        sort, index = torch.sort(torch.tensor(scores), -1, descending=True)
        sort_ = torch.softmax(sort, -1)

        eta = torch.minimum(torch.tensor(self.epsilon),
                            torch.sqrt(torch.tensor(self.epsilon)) * torch.exp(
                                (sort_ * torch.log(sort_)).sum()))

        indices_to_remove = torch.softmax(scores, -1) <= eta

        rest_token = scores < sort[..., :1]

        indices_to_remove = torch.logical_and(indices_to_remove, rest_token)

        scores = scores.masked_fill(indices_to_remove, self.filter_value)
        return scores


class AdaLogitsWarper(LogitsWarper):
    r"""
    :class:`transformers.LogitsWarper` that performs adaptive sampling (https://proceedings.mlr.press/v235/zhu24d.html).
    warning: current implementation of adaptive sampling doesn't support batch computation!
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        # the scores here is the raw logits, have to go through softmax for computing eta
        normalized_scores = torch.softmax(scores, -1)
        delta_conf = (1 / torch.log(torch.tensor(scores.shape[-1])) * (
                torch.sort(torch.tensor(normalized_scores), -1, descending=True)[0][..., 1:-1] * torch.log(
            torch.sort(torch.tensor(normalized_scores), -1, descending=True)[0][..., 1:-1]) + (
                        1 - torch.cumsum(torch.sort(torch.tensor(normalized_scores), -1, descending=True)[0],
                                         dim=-1)[..., 1:-1]) * torch.log(
            (1 - torch.cumsum(torch.sort(torch.tensor(normalized_scores), -1, descending=True)[0], dim=-1)[...,
                 1:-1]) / (
                    scores.shape[-1] - torch.arange(1, scores.shape[-1] - 1))) - (
                        1 - torch.cumsum(torch.sort(torch.tensor(normalized_scores), -1, descending=True)[0],
                                         dim=-1)[...,:-2]) * torch.log(
            (1 - torch.cumsum(torch.sort(torch.tensor(normalized_scores), -1, descending=True)[0], dim=-1)[
                 ...,:-2]) / (
                    scores.shape[-1] - torch.arange(scores.shape[-1] - 2))))).nan_to_num(0)


        sort, index = torch.sort(torch.tensor(scores), -1, descending=True)

        # be cautious that delta_conf is not monotonically decreasing!!
        thres = sort[..., 1:-1][delta_conf > self.epsilon]

        if len(thres) > 0:
            thres = thres[..., -1]
            indices_to_remove = scores < thres
        else:
            # keep at least the top-1 token
            indices_to_remove = scores < sort[..., 0, None]
...
class MirostatLogitsWarper(LogitsWarper):
    r"""
    :class:`transformers.LogitsWarper` that performs eta-sampling, i.e. restricting to the k highest probability elements.
    warning: current implementation doesn't support batch computation!
    ...
    def __init__(self, tau: float, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1):
        if not isinstance(tau, float) or tau <= 0:
            raise ValueError(f"`top_k` has to be a strictly positive float, but is {tau}")
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        max_surprise = 2 * self.tau
        # n = Vocabulary size

        sorted_logits, sorted_indices = torch.sort(normalized_scores, dim=-1, descending=True)
        prob_original = sorted_logits.squeeze().tolist()

        # Estimate s
        s = estimate_s(prob_original)
        # Compute k
        k = compute_k(scores.shape[-1], s, max_surprise)

        sort, index = torch.sort(torch.tensor(scores), -1, descending=True)

        indices_to_remove = scores < sort[..., k, None]
