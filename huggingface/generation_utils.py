...
class GenerationMixin:
    ...
    def _get_logits_warper(
        self, top_k: int = None, top_p: float = None, eta: float = None, ada: float = None, mirostat: float = None, tfs: float = None, temperature: float = None, num_beams: int = None, order: Tuple[int, int, int, int] = None
    ) -> LogitsProcessorList:
        """
        ...
        if ada is not None:
            warpers.append(AdaLogitsWarper(epsilon=ada, min_tokens_to_keep=(2 if num_beams > 1 else 1)))
        elif eta is not None:
            warpers.append(EtaLogitsWarper(epsilon=eta, min_tokens_to_keep=(2 if num_beams > 1 else 1)))
        elif mirostat is not None:
            warpers.append(MirostatLogitsWarper(tau=mirostat, min_tokens_to_keep=(2 if num_beams > 1 else 1)))
        ...
        # in the pretrained config, top-p might be used together with top_k
    ...
    def generate(
        ...
        reversed_greedy: Optional[bool] = False,
        ...
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            logits_warper = self._get_logits_warper(
                top_k=top_k, top_p=top_p, eta=eta, ada=ada, mirostat=mirostat, tfs=tfs, temperature=temperature, num_beams=num_beams, order=order
            )
            ...
            return self.sample(
                ...
                synced_gpus=synced_gpus,
                embs=embs,
                reversed_greedy=reversed_greedy,
                **model_kwargs,
            )
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if reversed_greedy:
                # set 0 prob to 1, so i can apply min to get the smallest non-zero value
                probs[probs==0] = 1
                next_tokens = probs.min(1)[1]
